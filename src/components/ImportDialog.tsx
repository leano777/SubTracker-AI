...
export function ImportDialog({
  ...
  const [duplicateStrategy, setDuplicateStrategy] = useState<"skip" | "replace" | "keep-both">(
    "skip"
  );
      ...
      <Card style={glassStyles}>
        <CardContent className="p-8">
          <div className="text-center">
            ...
            <h3
              className={`text-lg font-semibold mb-2 ${isDarkMode ? "text-gray-100" : "text-gray-900"}`}
            >
        ...
        <Card style={glassStyles}>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <Badge variant="outline">{getFileTypeInfo(selectedFile.name).type}</Badge>
            </div>
          </CardContent>
        </Card>
      )}
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>
                  Native SubTracker format with full support for subscriptions, payment cards, and
                  settings
                </p>
              </div>
            </div>
          ...
          <Alert>
            ...
            <AlertDescription className="text-sm">
              CSV files should include columns for: name, cost, billing_cycle, next_payment,
              category. Additional columns will be automatically mapped when possible.
            </AlertDescription>
          </Alert>
        </CardContent>
      </Card>
  ...
  const renderPreviewTab = () => {
    ...
    const validSubscriptions = preview.subscriptions.filter(
      (sub) => !sub._errors || sub._errors.length === 0
    );
    const invalidSubscriptions = preview.subscriptions.filter(
      (sub) => sub._errors && sub._errors.length > 0
    );
      ...
      <div className="space-y-6">
        ...
        {invalidSubscriptions.length > 0 && (
          <Card style={glassStyles}>
            <CardHeader>
              <CardTitle
                className={`text-lg text-red-600 ${isDarkMode ? "text-red-400" : "text-red-600"}`}
              >
                  ...
                  {invalidSubscriptions.map((subscription) => (
                    ...
                    >
                      ...
                      {subscription._errors && (
                        <ul
                          className={`text-sm mt-1 list-disc list-inside ${isDarkMode ? "text-red-300" : "text-red-600"}`}
                        >
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <div className="mt-6">
            ...
            <TabsContent value="preview" className="space-y-4">
              <ScrollArea className="h-[500px]">{renderPreviewTab()}</ScrollArea>
            </TabsContent>
          </div>
        </Tabs>
          ...
          <div className="flex gap-2">
            ...
            {preview && (
              <Button
                ...
                disabled={
                  isProcessing || (selectedSubscriptions.length === 0 && selectedCards.length === 0)
                }
                className="bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white"
              >
                {isProcessing ? (
