...
export const CalendarView = ({
  ...
  const handleMouseDown = (e: React.MouseEvent, subscription: Subscription, originalDate: Date) => {
    ...
    console.log(
      "🎯 Mouse down on subscription:",
      subscription.name,
      "on date:",
      getDateKey(originalDate)
    );
          ...
          <div
            ...
            className={`
              ...
              ${
                day.isToday
                  ? isStealthOps
                    ? "ring-2 ring-green-400 tactical-glow"
              ...
              {day.isThursday && day.weekNumber && day.isCurrentMonth && (
                <Badge
                  ...
                  className={`text-xs px-1 py-0 scale-75 ${
                    isStealthOps
                      ? "tactical-surface border border-blue-400 text-blue-400 font-mono tracking-wide"
                      : ""
                  }`}
                  style={isStealthOps ? { borderRadius: "0.125rem" } : undefined}
                >
            ...
            {isDropTarget && (
              <div
                className={`absolute inset-0 flex items-center justify-center z-30 ${
                  isStealthOps
                    ? "tactical-surface border-2 border-green-400 bg-green-900/50"
                    : "bg-green-100/95 rounded-lg"
                }`}
              >
                <div
            ...
            <div className="space-y-1 relative z-10">
              {day.subscriptions.slice(0, isMobile ? 2 : 3).map((subscription, subIndex) => (
                <div
                  ...
                  onMouseDown={
                    isDragEnabled ? (e) => handleMouseDown(e, subscription, day.date) : undefined
                  }
                  onClick={(e) => {
                    e.stopPropagation();
                  ...
                  className={`
                    ...
                    ${
                      isDragEnabled
                        ? "cursor-grab hover:scale-105 active:cursor-grabbing hover:shadow-lg"
                        : !isMoveModeEnabled
                ...
                >
                  <div className="flex items-center gap-1">
                    {isDragEnabled && <Move className="w-2 h-2 opacity-60 flex-shrink-0" />}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      >
                        {isMobile
                          ? subscription.name.substring(0, 8) +
                            (subscription.name.length > 8 ? "..." : "")
                          : subscription.name}
                      </div>
                      <div
                        className={`text-xs opacity-75 ${isStealthOps ? "font-mono tracking-wide" : ""}`}
                      >
  ...
  const WeekGridView = () => (
    <div className="space-y-4">
      ...
      <div
        className={`p-4 ${
          isStealthOps ? "tactical-surface border border-gray-600" : "rounded-lg bg-secondary/30"
        }`}
      >
          ...
          <div>
            ...
            >
              ...
              {formatCurrency(
                calendarDays.reduce(
                  (sum, day) =>
                    sum + day.subscriptions.reduce((daySum, sub) => daySum + sub.cost, 0),
                  0
                )
              )}{" "}
              total
      ...
      <div
        className={`grid gap-3 ${
          isMobile ? "grid-cols-1" : isTablet ? "grid-cols-2" : "grid-cols-7"
        }`}
      >
        {calendarDays.map((day, index) => {
          const isDropTarget =
            dropTargetDate && getDateKey(dropTargetDate) === getDateKey(day.date);
          const isDragEnabled = !!onUpdateSubscriptionDate && isMoveModeEnabled;
              ...
              <CardContent className={`p-3 ${isMobile ? "min-h-[120px]" : "min-h-[200px]"}`}>
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="text-center">
                    <div
                      className={`text-lg font-semibold ${
                        day.isToday
                          ? isStealthOps
                            ? "text-green-400 font-bold tactical-text-glow"
                  ...
                  {day.isToday && (
                    <Badge
                      ...
                      className={`text-xs ${
                        isStealthOps
                          ? "tactical-surface border border-green-400 text-green-400 font-mono tracking-wide"
                          : ""
                      }`}
                      style={isStealthOps ? { borderRadius: "0.125rem" } : undefined}
                    >
                ...
                {day.subscriptions.length > 0 && (
                  ...
                  >
                    ...
                    >
                      {isStealthOps
                        ? `[${day.subscriptions.length} SUB${day.subscriptions.length !== 1 ? "S" : ""}]`
                ...
                <div className="space-y-2 relative z-10">
                  {day.subscriptions.slice(0, isMobile ? 5 : 8).map((subscription, subIndex) => (
                    <div
                      ...
                      onMouseDown={
                        isDragEnabled
                          ? (e) => handleMouseDown(e, subscription, day.date)
                          : undefined
                      }
                      onClick={(e) => {
                        e.stopPropagation();
                    ...
                    >
                      <div className="flex items-center gap-2">
                        {isDragEnabled && (
                          <Move
                            className={`w-3 h-3 flex-shrink-0 ${isStealthOps ? "text-green-400" : "text-gray-500"}`}
                          />
                        )}
                        {subscription.logoUrl ? (
                          <ImageWithFallback
                          ...
                          >
                            <DollarSign
                              className={`w-2 h-2 ${isStealthOps ? "text-green-400" : "text-primary"}`}
                            />
                          </div>
                        )}
                        <div className="flex-1 min-w-0">
                        ...
                        >
                          {isStealthOps
                            ? subscription.status.toUpperCase().charAt(0)
                ...
                <div className="space-y-2 relative z-10">
                  {day.subscriptions.map((subscription, index) => (
                    ...
                    >
                      {isDragEnabled && (
                        <Move
                          className={`w-4 h-4 flex-shrink-0 ${isStealthOps ? "text-green-400" : "text-gray-500"}`}
                        />
                      )}
                      {subscription.logoUrl ? (
                        <ImageWithFallback
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {subscription.status === "cancelled" && (
                            <AlertCircle
                              className={`w-3 h-3 ${isStealthOps ? "text-red-400" : "text-red-500"}`}
                            />
                          )}
                          {subscription.status === "watchlist" && (
                            <Eye
                              className={`w-3 h-3 ${isStealthOps ? "text-yellow-400" : "text-yellow-500"}`}
                            />
                          )}
                        </div>
                        <div
  ...
  return (
    <div className="space-y-4">
      ...
      {dragState.isDragging && dragState.subscription && (
        ...
        >
          <div
            className={`flex items-center gap-2 ${isStealthOps ? "font-mono tracking-wide" : ""}`}
          >
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center justify-between w-full sm:w-auto">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <h3
              className={`text-lg font-semibold ${textColors.primary} min-w-[200px] text-center ${
                isStealthOps ? "font-mono tracking-wide tactical-text-glow" : ""
              }`}
            >
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
              ...
              className={`flex items-center gap-2 ${
                isStealthOps
                  ? `tactical-button border-gray-600 hover:border-green-400 font-mono tracking-wide ${
                      isMoveModeEnabled
                        ? "bg-green-600 text-black border-green-400 tactical-glow"
                        : "text-green-400"
                    }`
                  : ""
              }`}
              ...
              {isMoveModeEnabled ? (
                <>
                  ...
                  <span className="hidden sm:inline">
                    {isStealthOps ? "[MOVE MODE]" : "Move Mode"}
                  </span>
                </>
              ) : (
                <>
                  ...
                  <span className="hidden sm:inline">
                    {isStealthOps ? "[VIEW MODE]" : "View Mode"}
                  </span>
                </>
              )}
            </Button>
          ...
          <Sheet open={isFiltersOpen} onOpenChange={setIsFiltersOpen}>
            ...
            <SheetContent side="right" className={`w-64 ${isStealthOps ? "tactical-surface" : ""}`}>
              <SheetHeader>
                <SheetTitle
                ...
                <div>
                  <h3
                    className={`font-medium mb-2 ${textColors.primary} ${
                      isStealthOps
                        ? "font-mono tracking-wide text-green-400 tactical-text-glow"
                        : ""
                    }`}
                  >
