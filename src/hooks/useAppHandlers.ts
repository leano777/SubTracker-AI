...
export const useAppHandlers = ({
  ...
  const handleAddSubscription = useCallback(
    (subscriptionData: Omit<FullSubscription, "id">) => {
  ...
  const handleCancelSubscription = useCallback(
    (id: string) => {
      ...
      setSubscriptions((prev) => prev.map((sub) => (sub.id === id ? cancelledSubscription : sub)));
    },
  ...
  const handleReactivateSubscription = useCallback(
    (id: string) => {
      ...
      setSubscriptions((prev) =>
        prev.map((sub) => (sub.id === id ? reactivatedSubscription : sub))
      );
    },
    [subscriptions, setSubscriptions]
  );

  const handleActivateFromWatchlist = useCallback(
    (id: string) => {
      const subscription = subscriptions.find((sub) => sub.id === id);
      if (!subscription) return;
      ...
      setSubscriptions((prev) => prev.map((sub) => (sub.id === id ? activatedSubscription : sub)));
    },
    [subscriptions, setSubscriptions]
  );
  ...
  const handleEditCard = useCallback(
    (updatedCard: FullPaymentCard) => {
      if (updatedCard.isDefault) {
        setPaymentCards((prev) =>
          prev.map((card) =>
            card.id === updatedCard.id ? updatedCard : { ...card, isDefault: false }
          )
        );
      } else {
        setPaymentCards((prev) =>
  ...
  const handleDeleteCard = useCallback(
    (id: string) => {
      ...
      setSubscriptions((prev) =>
        prev.map((sub) => (sub.paymentCard === id ? { ...sub, paymentCard: undefined } : sub))
      );
    },
  ...
  const handleBulkEdit = useCallback(
    (subscriptionIds: string[], updates: Partial<FullSubscription>) => {
      setSubscriptions((prev) =>
        prev.map((sub) => (subscriptionIds.includes(sub.id) ? { ...sub, ...updates } : sub))
      );
    },
    [setSubscriptions]
  );
