...
class DataSyncManager {
    ...
    for (const endpoint of healthEndpoints) {
      try {
        ...
        if (response.ok) {
          ...
          console.log(`✅ Server is responding (${response.status} auth error on ${endpoint} is expected for public endpoints)`);
          return true;
        }
  ...
  private async makeRequest(endpoint: string, options: RequestInit = {}, retries: number = 3): Promise<any> {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (error instanceof DOMException && error.name === 'AbortError') {
          ...
          throw new Error(`Request timed out after ${timeoutMs}ms. The server may be experiencing issues.`);
        }
        ...
        if (error instanceof TypeError && error.message.includes('Failed to fetch')) {
          ...
          throw new Error('Network connection failed. Please check your internet connection or try again later.');
        }
  ...
  async loadFromCloud(): Promise<SyncResult> {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (errorMsg.includes('Supabase validation failed, but JWT is valid') || 
          ...
          return {
            success: true,
            data: response.data || { subscriptions: [], paymentCards: [], notifications: [], appSettings: {}, weeklyBudgets: [], metadata: {} },
  ...
  async saveToCloud(userData: Omit<UserData, 'metadata'>): Promise<SyncResult> {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (errorMsg.includes('Supabase validation failed, but JWT is valid') || 
            errorMsg.includes('Supabase secondary validation unavailable')) {
          console.info('ℹ️ Server reported secondary validation warning but save operation succeeded');
