...
export const createAutoBackup = (userId: string, data: {
  subscriptions: FullSubscription[];
  paymentCards: FullPaymentCard[];
  notifications: Notification[];
  appSettings: AppSettings;
...
export const getLatestBackup = (userId: string, backupType?: BackupData['backupType']): BackupData | null => {
  try {
...
export const detectDataLossAndRecover = (userId: string, currentData: {
  subscriptions: FullSubscription[];
  paymentCards: FullPaymentCard[];
}): BackupData | null => {
  ...
  if (currentData.subscriptions.length === 0 && currentData.paymentCards.length === 0) {
    ...
    if (recentBackup && (recentBackup.subscriptions.length > 0 || recentBackup.paymentCards.length > 0)) {
