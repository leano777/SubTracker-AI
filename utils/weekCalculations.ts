...
export function getThursdayWeeksForMonth(month: number, year: number): ThursdayWeek[] {
  ...
  thursdaysInMonth.forEach((thursday, index) => {
    ...
    if (month === 7 && year === 2025) { // August is month 7 (0-indexed)
      ...
      if (thursday.getTime() === august7th.getTime()) {
        ...
        const weeksSinceAug7 = Math.floor((thursday.getTime() - august7th.getTime()) / (7 * 24 * 60 * 60 * 1000)) + 1;
...
export function getCurrentThursdayWeek(): ThursdayWeek {
  ...
  const monthWeeks = getThursdayWeeksForMonth(currentThursday.getMonth(), currentThursday.getFullYear());
  const currentWeek = monthWeeks.find(week => 
    week.startDate.getTime() === currentThursday.getTime()
  );
  ...
  if (currentMonth === 7 && currentYear === 2025) { // August 2025
    ...
    if (currentThursday < august7th) {
      ...
      const julyWeek = julyWeeks.find(week => 
        week.startDate.getTime() === currentThursday.getTime()
      );
      if (julyWeek) {
