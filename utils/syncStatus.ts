...
export const getSyncStatusColor = (
  ...
  if (syncStatus?.type === 'error') {
    ...
    if (message.includes('offline') || message.includes('Server offline') || message.includes('Connection failed')) {
  ...
  if (syncStatus?.type === 'saving' || syncStatus?.type === 'loading') return isStealthOps ? 'text-blue-400' : 'text-blue-500';
...
export const getSyncStatusText = (
  ...
  return isOnline && cloudSyncEnabled ? (isStealthOps ? '[ONLINE]' : 'Online') : (isStealthOps ? '[OFFLINE]' : 'Offline');
};