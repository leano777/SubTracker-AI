...
function validateSubscription(data: any, index: number): { subscription: Subscription; errors: string[] } {
  const errors: string[] = [];
...
// Import CSV data
export function importCSVData(content: string, existingSubscriptions: Subscription[] = []): ImportPreview {
  ...
  try {
    ...
    if (columnMappings.name === -1) {
      preview.errors.push('Required column "name" not found. Expected columns: name, service, subscription, or title');
    }
    ...
    preview.errors.push(`Failed to parse CSV: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
...
// Import JSON data
export function importJSONData(content: string, existingSubscriptions: Subscription[] = [], existingCards: PaymentCard[] = []): ImportPreview {
  const preview: ImportPreview = {
    subscriptions: [],
    cards: [],
    errors: [],
  ...
  try {
    ...
    if (data.subscriptions && Array.isArray(data.subscriptions)) {
      data.subscriptions.forEach((sub: any, index: number) => {
        try {
          ...
          preview.errors.push(`Subscription ${index + 1}: ${error instanceof Error ? error.message : 'Invalid format'}`);
        }
      });
    }
    ...
    if (data.cards && Array.isArray(data.cards)) {
      data.cards.forEach((card: any, index: number) => {
        try {
          ...
          preview.errors.push(`Card ${index + 1}: ${error instanceof Error ? error.message : 'Invalid format'}`);
        }
      });
    }
    ...
    if (!data.subscriptions && !data.cards) {
      preview.warnings.push('No recognized data structure found. Expected "subscriptions" and/or "cards" arrays.');
    }
    
  } catch (error) {
    preview.errors.push(`Failed to parse JSON: ${error instanceof Error ? error.message : 'Invalid JSON format'}`);
  }
...
export function applyImport(
  ...
  const subscriptions = preview.subscriptions
    .filter(sub => selectedSubscriptions.length === 0 || selectedSubscriptions.includes(sub._importId))
  ...
  if (preview.duplicates.length > 0) {
    result.warnings.push(`${preview.duplicates.length} duplicates detected and handled according to your selection.`);
  }
