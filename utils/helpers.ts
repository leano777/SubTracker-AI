...
export function getWeekString(date: Date): string {
  ...
  const weekNumber = Math.ceil(((thursday.getTime() - firstThursday.getTime()) / (7 * 24 * 60 * 60 * 1000)) + 1);
...
export function calculateWeeklyAmount(price: number, frequency: string, variablePricing?: { averagePrice: number }): number {
  ...
  if (!frequency || typeof frequency !== 'string' || frequency.trim() === '') {
    console.warn('❌ Invalid frequency provided to calculateWeeklyAmount:', frequency, 'using fallback value of "monthly"');
  ...
  switch (normalizedFrequency) {
    ...
    default:
      console.warn(`❌ Unknown frequency "${frequency}" provided to calculateWeeklyAmount, defaulting to monthly`);
      return basePrice / 4.33; // Default to monthly
  }
}

export function calculateMonthlyAmount(price: number, frequency: string, variablePricing?: { averagePrice: number }): number {
  const basePrice = variablePricing?.averagePrice ?? price;
  
  // Handle undefined, null, or invalid frequency with more robust checking
  if (!frequency || typeof frequency !== 'string' || frequency.trim() === '') {
    console.warn('❌ Invalid frequency provided to calculateMonthlyAmount:', frequency, 'using fallback value of "monthly"');
    console.warn('  Subscription details - Price:', {
      price,
      frequency,
      variablePricing,
  ...
  switch (normalizedFrequency) {
    ...
    default:
      console.warn(`❌ Unknown frequency "${frequency}" provided to calculateMonthlyAmount, defaulting to monthly`);
      return basePrice;
  }
}

export function calculateYearlyAmount(price: number, frequency: string, variablePricing?: { averagePrice: number }): number {
  const basePrice = variablePricing?.averagePrice ?? price;
  
  // Handle undefined, null, or invalid frequency with more robust checking
  if (!frequency || typeof frequency !== 'string' || frequency.trim() === '') {
    console.warn('❌ Invalid frequency provided to calculateYearlyAmount:', frequency, 'using fallback value of "monthly"');
    console.warn('  Subscription details - Price:', {
      price,
      frequency,
      variablePricing,
  ...
  switch (normalizedFrequency) {
    ...
    default:
      console.warn(`❌ Unknown frequency "${frequency}" provided to calculateYearlyAmount, defaulting to monthly`);
      return basePrice * 12;
  }
}
...
export function safeCalculateWeeklyAmount(subscription: FullSubscription): number {
  try {
    ...
    console.log(`🔍 Safe weekly calculation for "${validated.name}": frequency="${validated.frequency}", price=${validated.price}`);
...
export function safeCalculateMonthlyAmount(subscription: FullSubscription): number {
  try {
    const validated = validateSubscriptionForCalculations(subscription);
    console.log(`🔍 Safe monthly calculation for "${validated.name}": frequency="${validated.frequency}", price=${validated.price}`);
...
export function safeCalculateYearlyAmount(subscription: FullSubscription): number {
  try {
    const validated = validateSubscriptionForCalculations(subscription);
    console.log(`🔍 Safe yearly calculation for "${validated.name}": frequency="${validated.frequency}", price=${validated.price}`);
...
export function calculateNextPaymentDate(lastPayment: Date, frequency: string): Date {
  ...
  if (!frequency || typeof frequency !== 'string') {
    console.warn('Invalid frequency provided to calculateNextPaymentDate:', frequency, 'defaulting to monthly');
...
export function validateSubscriptionForCalculations(subscription: FullSubscription): FullSubscription {
  ...
  if (!safeSub.frequency || typeof safeSub.frequency !== 'string' || !validFrequencies.includes(safeSub.frequency)) {
    console.warn(`⚠️ Invalid frequency "${safeSub.frequency}" (type: ${typeof safeSub.frequency}) in subscription "${safeSub.name}", fixing to monthly`);
  ...
  if (typeof safeSub.price !== 'number' || safeSub.price < 0 || isNaN(safeSub.price)) {
    console.warn(`⚠️ Invalid price "${safeSub.price}" (type: ${typeof safeSub.price}) in subscription "${safeSub.name}", fixing to 0`);
    safeSub.price = 0;
  }
  
  // Log the validated subscription for debugging
  console.log(`✅ Validated subscription "${safeSub.name}": frequency="${safeSub.frequency}", price=${safeSub.price}`);
...
export function calculateSubscriptionMetrics(subscription: FullSubscription) {
  ...
  console.log(`🔍 Calculating metrics for "${validatedSubscription.name}" with frequency: "${validatedSubscription.frequency}" and price: ${validatedSubscription.price}`);
  
  const weeklyAmount = calculateWeeklyAmount(validatedSubscription.price, validatedSubscription.frequency, validatedSubscription.variablePricing);
  const monthlyAmount = calculateMonthlyAmount(validatedSubscription.price, validatedSubscription.frequency, validatedSubscription.variablePricing);
  const yearlyAmount = calculateYearlyAmount(validatedSubscription.price, validatedSubscription.frequency, validatedSubscription.variablePricing);
...
export function calculateWeeklyBudgetAllocation(
  ...
  const allocations = categories.map(category => {
    const categorySubscriptions = subscriptions.filter(sub => 
      sub.status === 'active' && (sub.budgetCategory === category.id || sub.category === category.name)
    );
    ...
    const utilization = category.weeklyAllocation > 0 ? (requiredAmount / category.weeklyAllocation) * 100 : 0;
...
export function filterSubscriptionsByStatus(subscriptions: FullSubscription[], status: string[]): FullSubscription[] {
  return subscriptions.filter(sub => status.includes(sub.status));
}

export function filterSubscriptionsByCategory(subscriptions: FullSubscription[], categories: string[]): FullSubscription[] {
...
export function filterSubscriptionsByPriceRange(subscriptions: FullSubscription[], minPrice: number, maxPrice: number): FullSubscription[] {
...
export function sortSubscriptions(subscriptions: FullSubscription[], sortBy: string, sortOrder: 'asc' | 'desc' = 'asc'): FullSubscription[] {
...
// Search utility
export function searchSubscriptions(subscriptions: FullSubscription[], searchTerm: string): FullSubscription[] {
...
export function generateWeekOptions(weeksAhead: number = 8, weeksBehind: number = 4): Array<{ value: string; label: string; date: string }> {
  const options = [];
...
export function calculateSavingsPotential(subscriptions: FullSubscription[]): {
  ...
  const unusedSubscriptions = subscriptions.filter(sub => 
    sub.status === 'active' && 
    new Date(sub.dateAdded) < sixtyDaysAgo
...
export function validateSubscriptionData(subscription: Partial<FullSubscription>): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
