...
export function getCurrentPayPeriodThursday(date: Date = new Date()): Date {
  ...
  if (day === 4) { // Thursday - start of current pay period
    daysToCurrentThursday = 0;
  } else if (day === 5) { // Friday - current pay period started yesterday
    daysToCurrentThursday = -1;
  } else if (day === 6) { // Saturday - current pay period started 2 days ago
    daysToCurrentThursday = -2;
  } else if (day === 0) { // Sunday - current pay period started 3 days ago
    daysToCurrentThursday = -3;
  } else if (day === 1) { // Monday - current pay period started 4 days ago
    daysToCurrentThursday = -4;
  } else if (day === 2) { // Tuesday - current pay period started 5 days ago
    daysToCurrentThursday = -5;
  } else if (day === 3) { // Wednesday - current pay period started 6 days ago
...
function calculateSubscriptionOccurrences(
  ...
  for (let i = 0; i < maxForwardLookup; i++) {
    ...
    if (currentDate >= normalizedStart && currentDate <= normalizedEnd) {
      ...
      const existingDate = occurrences.find(occ => 
        normalizeDate(occ.date).getTime() === normalizeDate(currentDate).getTime()
      );
  ...
  const sortedOccurrences = occurrences
    ...
    .filter((occurrence, index, array) => 
      index === 0 || normalizeDate(occurrence.date).getTime() !== normalizeDate(array[index - 1].date).getTime()
    );
...
export function getUpcomingSubscriptions(
  ...
  console.log(`💰 Total amount: $${sortedSubscriptions.reduce((sum, sub) => sum + sub.cost, 0).toFixed(2)}`);
...
export function getPayPeriodSummary(
  ...
  const upcomingSubscriptions = getUpcomingSubscriptions(subscriptions, periodStart, periodEnd, includeAllStatuses);
  ...
  const upcomingChanges = getUpcomingPricingChanges(subscriptions)
    .filter(change => {
...
export function calculatePayPeriodRequirements(
  ...
  console.log(`🗓️ Current pay period: ${formatDate(currentPayPeriodStart)} to ${formatDate(currentPayPeriodEnd)} (Thu-Wed)`);
  ...
  thursdays.forEach((thursday, index) => {
    ...
    try {
      ...
      console.log(`✅ Period ${index + 1} complete: ${periodSubscriptions.length} subscriptions, $${totalRequired.toFixed(2)}`);
    } catch (error) {
  ...
  console.log(`💰 Total across all periods: $${requirements.reduce((sum, req) => sum + req.requiredAmount, 0).toFixed(2)}`);
  
  // Log summary of each period
  requirements.forEach((req, index) => {
    console.log(`   Period ${index + 1}: $${req.requiredAmount.toFixed(2)} (${req.subscriptions.length} subscriptions)`);
  });
...
export function getUpcomingPricingChanges(subscriptions: FullSubscription[]): Array<{
  ...
  subscriptions
    ...
    .forEach(subscription => {
      subscription.variablePricing!.upcomingChanges!
        .filter(change => parseDate(change.date) > normalizeDate(new Date()))
        .forEach(change => {
          changes.push({
            subscription,
            change,
  ...
  // Sort by date
  return changes.sort((a, b) => parseDate(a.change.date).getTime() - parseDate(b.change.date).getTime());
}
