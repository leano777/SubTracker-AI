...
export const createRecoveryBackup = (userId: string, data: any, sourceName: string) => {
  try {
    ...
    localStorage.setItem(recoveryBackupKey, JSON.stringify({
      ...data,
...
export const selectBestRecoverySource = (sources: any[]) => {
  ...
  return priorityOrder.map(type => 
    availableSources.find(s => s.type === type)
  ).filter(Boolean)[0] || availableSources[0];
};