...
function AppContent() {
  ...
  const handleUpdateSettings = useCallback((newSettings: typeof appSettings) => {
  ...
  }, [setSubscriptions, setPaymentCards, setNotifications, setAppSettings, defaultAppSettings, modalHandlers]);

  // Data recovery handler - FIXED: Proper validation and error handling
  const handleDataRecovered = useCallback((data: { 
    ...
    try {
      ...
      if (Array.isArray(data.subscriptions) && data.subscriptions.length > 0) {
        const importedSubs = data.subscriptions
          .filter((sub): sub is FullSubscription => 
            sub && 
            typeof sub === 'object' && 
      ...
      if (Array.isArray(data.cards) && data.cards.length > 0) {
        const importedCards = data.cards
          .filter((card): card is FullPaymentCard => 
            card && 
            typeof card === 'object' && 
  ...
  const handleViewSubscription = useCallback((subscription: FullSubscription) => {
  ...
  const handleUpdateSubscriptionDate = useCallback((subscriptionId: string, newDate: Date) => {
  ...
  const handleCardSave = useCallback((cardData: Omit<FullPaymentCard, 'id' | 'dateAdded'>) => {
  ...
  const handleDataSync = useCallback(async () => {
    if (!connectionState.isOnline || !connectionState.cloudSyncEnabled || !stableUserId || !triggerDataSync) {
      return;
    }
      ...
      <Dialog open={uiState.isFormOpen} onOpenChange={stableHandlers.setIsFormOpen}>
        ...
        >
          <DialogHeader>
            <DialogTitle>
              {editingState.editingSubscription ? 
                (editingState.editingSubscription.status === 'watchlist' ? 'Edit Watchlist Item' : 'Edit Subscription') : 
                (uiState.isWatchlistMode ? 'Add to Watchlist' : 'Add New Subscription')
              }
            </DialogTitle>
          ...
          <SubscriptionForm
            ...
            cards={paymentCards || []}
            onSave={editingState.editingSubscription ? handleEditSubscription : handleAddSubscription}
            onCancel={closeForm}
        ...
        >
          <DialogHeader>
            ...
            <DialogDescription id="settings-dialog-description">
              Customize your SubTracker experience, manage preferences, and configure advanced settings.
            </DialogDescription>
          </DialogHeader>
          <AdvancedSettingsTab
            settings={appSettings || defaultAppSettings}
