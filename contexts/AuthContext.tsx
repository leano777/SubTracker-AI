...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const isAuthSessionError = (error: any): boolean => {
    ...
    return (errorMessage.includes('refresh') && 
           (errorMessage.includes('token') || errorMessage.includes('Token')) &&
           (errorMessage.includes('Invalid') || errorMessage.includes('Not Found') || errorMessage.includes('expired'))) ||
      ...
      if (error) {
        ...
        if (error.message.includes('provider is not enabled') || 
          ...
          if (error.message.includes('redirect_uri_mismatch')) {
            configError += 'The redirect URI in Google Cloud Console doesn\'t match the expected URL. ';
