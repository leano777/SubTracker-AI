...
export function QuickActionButton({ 
  ...
  const tabs = [
    ...
    { id: 'subscriptions', label: 'Subscriptions', icon: Grid3X3, color: 'green', badge: subscriptionsCount },
    { id: 'planning', label: 'Planning', icon: Calendar, color: 'purple' },
    { id: 'intelligence', label: 'Intelligence', icon: Brain, color: 'orange', badge: aiInsightsCount }
  ];
  ...
  const getGlassStyles = () => {
    if (isStealthOps) {
      return {
        ...
        backgroundImage: 'linear-gradient(135deg, rgba(0, 255, 0, 0.05) 0%, rgba(0, 255, 0, 0.02) 100%)'
      };
    }
    return {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-40">
          ...
          <div className="absolute bottom-16 right-0 mb-2">
            <div 
              className={`p-4 shadow-xl min-w-[280px] ${
                isStealthOps ? 'tactical-surface' : 'rounded-2xl border border-white/20 dark:border-gray-700/30'
              }`}
              style={{
                ...getGlassStyles(),
            ...
            >
              {/* Quick Actions */}
              <div className="mb-4">
                <h3 className={`text-sm font-medium mb-3 ${textColors.secondary} ${
      ...
      >
        {isExpanded ? (
          <X className={`w-6 h-6 ${isStealthOps ? (isExpanded ? 'text-red-400' : 'text-green-400') : ''}`} />
        ) : (
