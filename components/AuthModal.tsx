...
import { Mail, Lock, User, Chrome, Eye, EyeOff, Loader2, AlertCircle, Info, ExternalLink } from 'lucide-react';
...
export function AuthModal({ isOpen, onClose }: AuthModalProps) {
  ...
  const { signIn, signUp, signInWithGoogle, resetPassword, loading, error, googleAuthAvailable } = useAuth();
  ...
  const getDescription = () => {
    switch (mode) {
      case 'signin': return 'Sign in to your SubTracker AI account to access your subscription management dashboard';
      case 'signup': return 'Join thousands of users managing their subscriptions with AI-powered insights';
      ...
      <DialogContent className="max-w-md" aria-describedby={AUTH_DIALOG_DESCRIPTION_ID}>
        ...
        <div className="space-y-6">
          ...
          {currentError && !showGoogleSetup && (
            <Alert variant={currentError.includes('sent') || currentError.includes('created') ? 'default' : 'destructive'}>
