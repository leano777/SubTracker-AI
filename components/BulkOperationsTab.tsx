...
import { Upload, Download, FileText, Search, Filter, CheckCircle, AlertCircle, Copy, Trash2, Edit } from 'lucide-react';
...
export function BulkOperationsTab({ 
  ...
  const handleBulkDelete = () => {
    if (confirm(`Are you sure you want to delete ${selectedIds.length} subscriptions? This action cannot be undone.`)) {
    ...
    <div className="space-y-6">
      ...
      {importStatus === 'error' && (
        <Alert variant="destructive">
          ...
          <AlertDescription>Import failed. Please check your data format and try again.</AlertDescription>
        </Alert>
      )}
      ...
      {showBulkActions && selectedIds.length > 0 && (
        <Card className="border-primary/20 bg-primary/5">
          <CardHeader>
            ...
            <CardDescription>
              Monthly cost: {formatCurrency(selectionStats.totalCost)} • {selectionStats.activeCount} active
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 gap-3">
              <Button onClick={() => handleExport('json')} className="h-auto p-4 flex-col items-start">
              ...
              <Button onClick={() => handleExport('csv')} variant="outline" className="h-auto p-4 flex-col items-start">
                <div className="flex items-center space-x-2 mb-2 w-full">
                  <FileText className="w-5 h-5" />
              ...
              <Button onClick={copyShareableData} variant="outline" className="h-auto p-4 flex-col items-start">
                <div className="flex items-center space-x-2 mb-2 w-full">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Use filters to find specific subscriptions, then select multiple items for bulk operations
          </CardDescription>
        </CardHeader>
        <CardContent>
          {/* Filters */}
              ...
              {filteredSubscriptions.length === 0 ? (
                ...
                <div className="divide-y">
                  {filteredSubscriptions.map((subscription) => (
                    <div key={subscription.id} className="flex items-center space-x-3 p-3 hover:bg-muted/50">
                      <Checkbox
