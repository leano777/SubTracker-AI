...
import { Brain, TrendingUp, TrendingDown, AlertTriangle, Lightbulb, Target, Zap, DollarSign, Calendar, PieChart, BarChart3, Sparkles } from 'lucide-react';
...
export function AIInsightsTab({ subscriptions, cards }: AIInsightsTabProps) {
  ...
  const generateInsights = useMemo(() => {
    ...
    const monthlyTotal = activeSubscriptions.reduce((total, sub) => {
      const monthly = sub.billingCycle === 'monthly' ? sub.cost : 
    ...
    const serviceCategories = activeSubscriptions.reduce((acc, sub) => {
    ...
    const expensiveServices = activeSubscriptions
      .map(sub => ({
        ...sub,
        monthlyEquivalent: sub.billingCycle === 'monthly' ? sub.cost : 
                          sub.billingCycle === 'quarterly' ? sub.cost / 3 : 
                          sub.cost / 12
      }))
    ...
    if (cards.length > 1) {
      const cardUsage = activeSubscriptions.reduce((acc, sub) => {
        if (sub.cardId) {
  ...
  const totalPotentialSavings = insights.reduce((total, insight) => 
    total + (insight.potentialSavings || 0), 0
  );
  ...
  const getInsightColor = (type: string) => {
    if (isStealthOps) {
      switch (type) {
        case 'optimization': return 'text-blue-400 tactical-surface border border-blue-400';
        case 'warning': return 'text-orange-400 tactical-surface border border-orange-400';
        case 'opportunity': return 'text-green-400 tactical-surface border border-green-400';
        case 'trend': return 'text-purple-400 tactical-surface border border-purple-400';
        case 'prediction': return 'text-indigo-400 tactical-surface border border-indigo-400';
        default: return 'text-gray-400 tactical-surface border border-gray-600';
      }
    }
    switch (type) {
      case 'optimization': return 'text-blue-600 bg-blue-50 dark:bg-blue-950';
      case 'warning': return 'text-orange-600 bg-orange-50 dark:bg-orange-950';
      case 'opportunity': return 'text-green-600 bg-green-50 dark:bg-green-950';
      case 'trend': return 'text-purple-600 bg-purple-50 dark:bg-purple-950';
      case 'prediction': return 'text-indigo-600 bg-indigo-50 dark:bg-indigo-950';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className={`flex items-center space-x-2 ${textColors.primary} ${
        ...
        {isAnalyzing && (
          <div className={`flex items-center space-x-2 ${
            ...
            <div className={`animate-spin h-4 w-4 border-b-2 ${
              isStealthOps 
                ? 'border-green-400 tactical-glow' 
                : 'rounded-full border-purple-600'
            }`}
             style={isStealthOps ? { borderRadius: '0.125rem' } : undefined}
            ></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className={`${
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2 mb-2">
              <DollarSign className={`w-4 h-4 ${isStealthOps ? 'text-green-400' : 'text-green-600'}`} />
              <div className={`text-sm ${
                isStealthOps 
        ...
        <Card className={`${
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2 mb-2">
              <AlertTriangle className={`w-4 h-4 ${isStealthOps ? 'text-orange-400' : 'text-orange-600'}`} />
              <div className={`text-sm ${
                isStealthOps 
              ...
              {filteredInsights.map((insight) => {
                ...
                return (
                  <Card 
                    ...
                    style={{ 
                      borderLeftColor: insight.impact === 'high' ? (isStealthOps ? '#ff0000' : '#ef4444') : 
                                      insight.impact === 'medium' ? (isStealthOps ? '#ffff00' : '#f59e0b') : 
                                      (isStealthOps ? '#00ff00' : '#10b981'),
                  ...
                  >
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex items-start space-x-3">
                          ...
                          <div className="flex-1">
                            <CardTitle className={`text-base ${textColors.primary} ${
                              isStealthOps ? 'font-mono tracking-wide' : ''
                            }`}>
                              {isStealthOps ? `[${insight.title.toUpperCase()}]` : insight.title}
                            </CardTitle>
                            <CardDescription className={`mt-1 ${textColors.muted} ${
                              isStealthOps ? 'font-mono tracking-wide' : ''
                            }`}>
                        ...
                        <div className="flex items-center space-x-2">
                          <Badge 
                            ...
                            style={isStealthOps ? { 
                              ...
                              borderColor: insight.impact === 'high' ? '#ff0000' : insight.impact === 'medium' ? '#ffff00' : '#00ff00',
                              color: insight.impact === 'high' ? '#ff0000' : insight.impact === 'medium' ? '#ffff00' : '#00ff00'
                            } : undefined}
                          >
                          ...
                          <Badge 
                            variant="outline"
                            className={`${
                              isStealthOps ? 'font-mono tracking-wide tactical-surface border border-gray-600' : ''
                            }`}
                            style={isStealthOps ? { borderRadius: '0.125rem' } : undefined}
                          >
                            {isStealthOps 
                    ...
                    <CardContent className="pt-0">
                      ...
                      {insight.actionItems && insight.actionItems.length > 0 && (
                        <div>
                          <h4 className={`font-medium mb-2 ${textColors.primary} ${
            ...
            {filteredInsights.length === 0 && (
              <Card className={isStealthOps ? 'tactical-surface' : ''}>
                <CardContent className="text-center py-8">
                  ...
                  <p className={`${textColors.muted} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                    {isAnalyzing 
                      ? (isStealthOps ? '[ANALYZING YOUR DATA...]' : 'Analyzing your data...') 
                      : (isStealthOps ? '[NO INSIGHTS OF THIS TYPE FOUND.]' : 'No insights of this type found.')
                    }
                  </p>
                </CardContent>
              </Card>
                  ...
                  {spendingPatterns.map((pattern, index) => (
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        ...
                        <Badge 
                          variant={pattern.trend === 'up' ? 'destructive' : pattern.trend === 'down' ? 'default' : 'secondary'}
                          className={`${
                            isStealthOps ? 'font-mono tracking-wide tactical-surface border' : ''
                          }`}
                          style={isStealthOps ? { 
                            borderRadius: '0.125rem',
                            borderColor: pattern.trend === 'up' ? '#ff0000' : pattern.trend === 'down' ? '#00ff00' : '#666666',
                            color: pattern.trend === 'up' ? '#ff0000' : pattern.trend === 'down' ? '#00ff00' : '#cccccc'
                          } : undefined}
                        >
                      ...
                      <div className="text-right">
                        ...
                        <div className={`text-xs ${textColors.muted} ${
                          ...
                          {isStealthOps ? '[PREDICTED: ' : 'Predicted: '}{formatCurrency(pattern.prediction)}{isStealthOps ? ']' : ''}
                        </div>
                      </div>
                    </div>
                  ))}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <div className={`text-sm ${textColors.muted} ${
                        ...
                        {formatCurrency(category.currentSpend)}{isStealthOps ? '[/MONTH]' : '/month'}
                      </div>
                    </div>
                  </div>
                </CardHeader>
              ...
              <CardContent>
                <Alert className={isStealthOps ? 'tactical-surface border border-green-400' : ''}>
                  ...
                  <AlertDescription className={`${
                    isStealthOps ? 'font-mono tracking-wide text-green-300' : ''
                  }`}>
                    {isStealthOps 
                ...
                <div className="mt-6 space-y-4">
                  {insights.filter(insight => insight.type === 'prediction').map((prediction) => (
                    <div 
                      key={prediction.id} 
                      className={`p-4 border-l-4 ${
                        isStealthOps ? 'tactical-surface border-indigo-400' : 'bg-indigo-50 dark:bg-indigo-950 rounded-lg border-indigo-500'
                      }`}
                      style={isStealthOps ? { borderRadius: '0.125rem' } : undefined}
                    >
                      ...
                      <p className={`text-sm mb-3 ${textColors.secondary} ${
                        isStealthOps ? 'font-mono tracking-wide' : ''
                      }`}>
                        {isStealthOps ? `[${prediction.description.toUpperCase()}]` : prediction.description}
                      </p>
                      <div className={`text-xs ${textColors.muted} ${
                        isStealthOps ? 'font-mono tracking-wide' : ''
                      }`}>
