  ...
  return (
    <div 
      ...
      style={{
        ...
        borderBottomColor: isStealthOps ? '#333333' : (isDarkMode ? 'rgba(75, 85, 99, 0.2)' : 'rgba(229, 231, 235, 0.5)')
      }}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 sm:gap-6 lg:gap-8 min-w-0">
            <h1 className={`text-lg sm:text-xl lg:text-2xl font-semibold ${safeTextColors.primary} drop-shadow-sm flex-shrink-0`}>
              <span className={`${
                isStealthOps 
            ...
            <nav 
              ...
              style={{ 
                backgroundColor: isStealthOps 
                  ? 'rgba(0, 0, 0, 0.9)' 
                  : (isDarkMode ? 'rgba(31, 41, 55, 0.3)' : 'rgba(255, 255, 255, 0.3)'),
              ...
              <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
                <SheetTrigger asChild>
                  <Button
                    ...
                    style={isStealthOps ? { borderRadius: '0.125rem' } : undefined}
                  >
                    <Menu className={`w-4 h-4 ${isStealthOps ? 'text-green-400' : safeTextColors.primary}`} />
                  </Button>
                </SheetTrigger>
                <SheetContent 
                  side="left" 
                ...
                >
                  <SheetHeader className="py-6">
                    ...
                    <SheetDescription id="mobile-navigation-description" className={safeTextColors.muted}>
                      {isStealthOps ? '[NAVIGATE BETWEEN APP SECTIONS]' : 'Navigate between app sections'}
                    </SheetDescription>
                  </SheetHeader>
                  <div className="pb-6">
                    <MobileNavigation 
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
            ...
            <div className={`flex items-center gap-1 ${getSyncStatusColor(isOnline, cloudSyncEnabled, isAuthenticated, syncStatus, isStealthOps)}`} title={getSyncStatusText(isAuthenticated, isOnline, cloudSyncEnabled, syncStatus, isStealthOps)}>
              ...
              <Badge 
                ...
                style={{ 
                  backgroundColor: getSyncStatusColor(isOnline, cloudSyncEnabled, isAuthenticated, syncStatus, isStealthOps).includes('red') ? 'rgba(239, 68, 68, 0.1)' :
                                 getSyncStatusColor(isOnline, cloudSyncEnabled, isAuthenticated, syncStatus, isStealthOps).includes('orange') ? 'rgba(249, 115, 22, 0.1)' :
                                 getSyncStatusColor(isOnline, cloudSyncEnabled, isAuthenticated, syncStatus, isStealthOps).includes('yellow') ? 'rgba(245, 158, 11, 0.1)' :
                                 getSyncStatusColor(isOnline, cloudSyncEnabled, isAuthenticated, syncStatus, isStealthOps).includes('blue') ? 'rgba(59, 130, 246, 0.1)' :
                                 getSyncStatusColor(isOnline, cloudSyncEnabled, isAuthenticated, syncStatus, isStealthOps).includes('green') ? 'rgba(34, 197, 94, 0.1)' :
                                 'rgba(107, 114, 128, 0.1)',
                  borderRadius: isStealthOps ? '0.125rem' : undefined
                }}
              >
                {getSyncStatusText(isAuthenticated, isOnline, cloudSyncEnabled, syncStatus, isStealthOps)}
              </Badge>
            </div>
            ...
            <Button
              ...
              className={`h-9 w-9 p-0 backdrop-blur-sm transition-all duration-300 ${
                isStealthOps 
                  ? 'tactical-button border border-gray-600 hover:border-green-400 text-green-400' 
                  : (isDarkMode ? 'bg-gray-700/50 hover:bg-gray-600 rounded-lg' : 'bg-white/50 hover:bg-white/70 rounded-lg')
              }`}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  ...
                  style={isStealthOps ? { borderRadius: '0.125rem' } : undefined}
                >
                  <Avatar className={`h-7 w-7 ring-2 ${
                    isStealthOps ? 'ring-green-400' : (isDarkMode ? 'ring-gray-600' : 'ring-gray-200')
                  }`}
                   style={isStealthOps ? { borderRadius: '0.125rem' } : undefined}
                  >
                  ...
                  <span className={`text-sm font-medium ${safeTextColors.primary} hidden sm:block max-w-20 lg:max-w-32 truncate`}>
              ...
              >
                <DropdownMenuLabel className={`font-normal ${safeTextColors.onGlass}`}>
                  <div className="flex flex-col space-y-1">
                    ...
                    {lastSyncTime && (
                      <p className={`text-xs leading-none ${safeTextColors.muted}`}>
                        {isStealthOps ? '[SYNC: ' : 'Last sync: '}{new Date(lastSyncTime).toLocaleTimeString()}{isStealthOps ? ']' : ''}
                      </p>
                    )}
                  </div>
                </DropdownMenuLabel>
