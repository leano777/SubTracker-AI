...
import { formatCurrency, calculateMonthlyAmount, validateSubscriptionForCalculations } from '../utils/helpers';
...
export function IntelligenceTab({ subscriptions, cards, onAutomationTrigger }: IntelligenceTabProps) {
  ...
  const totalMonthly = activeSubscriptions.reduce((total, sub) => {
    ...
    return total + calculateMonthlyAmount(validatedSub.price, validatedSub.frequency, validatedSub.variablePricing);
  }, 0);

  // Mock AI stats
  const aiStats = {
    insightsGenerated: 7,
    potentialSavings: 247.50,
    automationRules: 3,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className={`text-xl sm:text-2xl font-semibold flex items-center space-x-2 ${textColors.primary} ${
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex-1">
                ...
                <p className={`text-xs mt-1 ${
                  ...
                  {isStealthOps ? `[${analysisProgress}% COMPLETE]` : `${analysisProgress}% complete`}
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className={`${
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2 mb-2">
              <Brain className={`w-4 h-4 ${isStealthOps ? 'text-purple-400' : 'text-purple-600'}`} />
        ...
        <Card className={`${
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2 mb-2">
              <TrendingUp className={`w-4 h-4 ${isStealthOps ? 'text-green-400' : 'text-green-600'}`} />
              <div className={`text-sm ${
                isStealthOps 
                  ? 'text-green-300 font-mono tracking-wide' 
                  : 'text-green-700 dark:text-green-300'
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <div className={`p-2 ${
               ...
               style={isStealthOps ? { borderRadius: '0.125rem' } : undefined}
              >
                <Brain className={`w-5 h-5 ${isStealthOps ? 'text-purple-400' : 'text-purple-600'}`} />
              </div>
              <div>
                <h3 className={`font-medium ${
                  isStealthOps 
      ...
      <Card className={isStealthOps ? 'tactical-surface' : ''}>
        <CardHeader>
          <CardTitle className={`flex items-center space-x-2 ${textColors.primary} ${
            isStealthOps ? 'font-mono tracking-wide tactical-text-glow' : ''
          }`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            >
              ...
              <p className={`text-sm text-left ${
                isStealthOps 
                  ? 'text-gray-300 font-mono tracking-wide' 
                  : 'text-muted-foreground'
              }`}>
                {isAnalyzing 
                  ? (isStealthOps ? '[ANALYZING SUBSCRIPTIONS...]' : 'Analyzing subscriptions...') 
                  : (isStealthOps ? '[ANALYZE ALL SUBSCRIPTIONS FOR OPTIMIZATION OPPORTUNITIES]' : 'Analyze all subscriptions for optimization opportunities')
                }
              </p>
            </Button>
      ...
      <Dialog open={showAutomationSetup} onOpenChange={setShowAutomationSetup}>
        ...
        >
          <DialogHeader>
            <DialogTitle className={`flex items-center space-x-2 ${textColors.primary} ${
            ...
            <DialogDescription id="automation-setup-description" className={`${textColors.muted || 'text-gray-600'} ${
              isStealthOps ? 'font-mono tracking-wide' : ''
            }`}>
              {isStealthOps 
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              >
                ...
                <p className={`text-sm ${textColors.muted || 'text-gray-600'} mt-1 ${
                  isStealthOps ? 'font-mono tracking-wide' : ''
                }`}>
                  {isStealthOps 
        ...
        >
          <DialogHeader>
            <DialogTitle className={`flex items-center space-x-2 ${textColors.primary} ${
              ...
              <span>{isStealthOps ? '[OPTIMIZATION RECOMMENDATIONS]' : 'Optimization Recommendations'}</span>
            </DialogTitle>
            <DialogDescription id="optimization-results-description" className={`${textColors.muted || 'text-gray-600'} ${
              isStealthOps ? 'font-mono tracking-wide' : ''
            }`}>
              {isStealthOps 
          ...
          <div className="space-y-4">
            <div className={`p-4 border ${
              isStealthOps ? 'tactical-surface border-green-400' : 'rounded-lg bg-green-50 border-green-200'
            }`}
             style={isStealthOps ? { borderRadius: '0.125rem' } : undefined}
            >
              <h3 className={`font-medium ${
              ...
              <p className={`text-2xl font-bold ${
                ...
                {formatCurrency(optimizationRecommendations.reduce((total, rec) => total + rec.savings, 0))}
                <span className={`text-sm ${
                  isStealthOps ? 'text-green-300' : 'text-green-700'
                }`}>
            ...
            <div className="space-y-3">
              {optimizationRecommendations.map((recommendation) => (
                <div key={recommendation.id} className={`p-4 border ${
                  isStealthOps ? 'tactical-surface border-gray-600' : 'rounded-lg'
                }`}
                 style={isStealthOps ? { borderRadius: '0.125rem' } : undefined}
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <h4 className={`font-medium ${textColors.primary} ${
                        ...
                        {isStealthOps ? `[${recommendation.title.toUpperCase()}]` : recommendation.title}
                      </h4>
                      <p className={`text-sm ${textColors.muted || 'text-gray-600'} mt-1 ${
                        isStealthOps ? 'font-mono tracking-wide' : ''
                      }`}>
                      ...
                      <div className="flex items-center space-x-4 mt-2">
                        <Badge variant={recommendation.impact === 'High' ? 'default' : 'secondary'} className={`${
                          isStealthOps ? 'tactical-surface border border-gray-600 font-mono tracking-wide' : ''
                        }`}>
                          {isStealthOps ? `[${recommendation.impact.toUpperCase()} IMPACT]` : `${recommendation.impact} Impact`}
                        </Badge>
                        <Badge variant="outline" className={`${
                          isStealthOps ? 'tactical-surface border border-gray-600 font-mono tracking-wide' : ''
                        }`}>
                          {isStealthOps ? `[${recommendation.effort.toUpperCase()} EFFORT]` : `${recommendation.effort} Effort`}
                        </Badge>
                      </div>
                    </div>
                    <div className="text-right ml-4">
                      ...
                      <div className={`text-xs ${textColors.muted || 'text-gray-600'} ${
                        isStealthOps ? 'font-mono tracking-wide' : ''
                      }`}>
