      ...
      >
        ...
        <div className="space-y-6">
          {recoveryStatus === 'scanning' && (
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                <RefreshCw className={`w-5 h-5 animate-spin ${isStealthOps ? 'text-green-400' : 'text-blue-500'}`} />
                <span className={textColors.onGlass}>
                  {isStealthOps ? '[SCANNING FOR RECOVERABLE DATA...]' : 'Scanning for recoverable data...'}
                </span>
              </div>
          ...
          {recoveryStatus === 'results' && (
            <div className="space-y-4">
              <div className={`p-4 rounded-lg ${isStealthOps ? 'tactical-surface border border-blue-400' : 'bg-blue-50 dark:bg-blue-900/20 border border-blue-200'}`}>
                <div className="flex items-start gap-3">
                  <CheckCircle className={`w-5 h-5 mt-0.5 ${isStealthOps ? 'text-blue-400' : 'text-blue-500'}`} />
                  <div>
              ...
              <div className="space-y-3">
                {recoverySources.map((source, index) => (
                  <div
                    ...
                    onClick={() => source.status === 'found' ? setSelectedSource(source) : undefined}
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className={`text-sm ${textColors.muted} mb-2`}>
                            {isStealthOps ? `[${source.description.toUpperCase()}]` : source.description}
                          </p>
                          {source.status === 'found' && (
                      ...
                      {selectedSource === source && (
                        <CheckCircle className={`w-5 h-5 ${isStealthOps ? 'text-green-400' : 'text-blue-500'}`} />
                      )}
                    </div>
                  </div>
                ))}
          ...
          {recoveryStatus === 'recovering' && (
            <div className="space-y-4 text-center">
              <div className="flex items-center justify-center gap-3">
                <RefreshCw className={`w-6 h-6 animate-spin ${isStealthOps ? 'text-green-400' : 'text-blue-500'}`} />
          ...
          {recoveryStatus === 'complete' && (
            <div className="space-y-4 text-center">
              <div className="flex items-center justify-center gap-3">
                <CheckCircle className={`w-8 h-8 ${isStealthOps ? 'text-green-400' : 'text-green-500'}`} />
