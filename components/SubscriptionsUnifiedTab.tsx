...
export function SubscriptionsUnifiedTab({
  ...
  const filteredSubscriptions = useMemo(() => {
    ...
    return filtered;
  }, [subscriptions, searchTerm, selectedCategory, selectedStatus, selectedCardStatus, selectedAutomationStatus, sortBy, sortOrder, currentTab]);
  ...
  const stats = useMemo(() => {
    ...
    const cardsLinked = subscriptions.filter(sub => sub.linkedCard !== undefined && sub.linkedCard !== null).length;
                ...
                <div className="text-right">
                  ...
                  {daysUntil !== null && (
                    <div className={`text-xs ${
                      daysUntil <= 3 ? 'text-red-600' : daysUntil <= 7 ? 'text-orange-600' : 'text-muted-foreground'
                    }`}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-primary">{formatCurrency(stats.totalMonthly)}</div>
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <Button 
          ...
          size="sm"
          onClick={() => {
            setSelectedAutomationStatus(selectedAutomationStatus === 'disabled' ? 'all' : 'disabled');
            setCurrentTab('active');
          }}
          className="h-7 text-xs"
        >
        ...
        <Button 
          variant={selectedCardStatus === 'linked' && selectedAutomationStatus === 'enabled' ? 'default' : 'outline'} 
          size="sm"
          onClick={() => {
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {(selectedCategory !== 'all' || selectedCardStatus !== 'all' || selectedAutomationStatus !== 'all') && (
            <div className="flex items-center gap-2">
              <Button 
                variant="ghost" 
                size="sm"
              ...
              <div className="text-xs text-muted-foreground">
                {[
                  ...
                  selectedCardStatus !== 'all' && `Card: ${selectedCardStatus === 'linked' ? 'Linked' : 'Not Linked'}`,
                  selectedAutomationStatus !== 'all' && `Auto: ${selectedAutomationStatus === 'enabled' ? 'On' : 'Off'}`
                ].filter(Boolean).join(' • ')}
              </div>
            </div>
          )}
            ...
            <Card>
              <CardContent className="p-12 text-center">
                ...
                <p className="text-muted-foreground mb-4">
                  {searchTerm || selectedCategory !== 'all' || selectedCardStatus !== 'all' || selectedAutomationStatus !== 'all'
                    ? (
                        <>
                          ...
                          {(selectedCardStatus !== 'all' || selectedAutomationStatus !== 'all') && (
                            <span className="block mt-2 text-sm">
                              Active filters: {[
                                ...
                                selectedCardStatus !== 'all' && `Card: ${selectedCardStatus === 'linked' ? 'Linked' : 'Not Linked'}`,
                                selectedAutomationStatus !== 'all' && `Automation: ${selectedAutomationStatus === 'enabled' ? 'Enabled' : 'Disabled'}`
                              ].filter(Boolean).join(' • ')}
                            </span>
                          )}
                        </>
