...
import { calculateMonthlyAmount, formatCurrency, validateSubscriptionForCalculations, safeCalculateMonthlyAmount } from '../utils/helpers';
...
export function DashboardTab({ subscriptions, cards, settings, notifications }: DashboardTabProps) {
  ...
  // Quick stats cards
  const QuickStatsCard = ({ title, value, change, icon: Icon, trend, color = 'blue' }: {
    title: string;
    value: string;
    change?: string;
    icon: any;
      ...
      <CardContent className="p-4 sm:p-6">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            <p className={`text-sm ${isStealthOps ? 'text-gray-400 font-mono tracking-wide' : 'text-muted-foreground'}`}>
              {isStealthOps ? `[${title.toUpperCase()}]` : title}
            </p>
            <p className={`text-xl sm:text-2xl font-bold ${textColors.primary} ${isStealthOps ? 'font-mono tracking-wide tactical-text-glow' : ''}`}>
              {value}
            </p>
            {change && (
              <div className={`flex items-center gap-1 text-xs ${
                trend === 'up' 
                  ? (isStealthOps ? 'text-green-400' : 'text-green-600') 
                  : trend === 'down' 
                    ? (isStealthOps ? 'text-red-400' : 'text-red-600') 
                    : textColors.muted
  ...
  const SubscriptionCard = ({ subscription, showCategory = false }: { subscription: FullSubscription; showCategory?: boolean }) => (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className={`font-medium truncate ${textColors.primary} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
          {subscription.name}
        </div>
        <div className={`text-sm ${textColors.muted} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
          {isStealthOps 
            ...
            : (showCategory ? subscription.category : subscription.frequency)
          }
        </div>
      </div>
      
      <div className="text-right">
        <div className={`font-medium ${textColors.primary} ${isStealthOps ? 'font-mono tracking-wide tactical-text-glow' : ''}`}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className={`text-xl sm:text-2xl font-semibold ${textColors.primary} ${
            isStealthOps ? 'font-mono tracking-wide tactical-text-glow' : ''
          }`}>
            {isStealthOps ? '[DASHBOARD OVERVIEW]' : 'Dashboard Overview'}
          </h2>
          <p className={`text-sm ${textColors.muted} mt-1 ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
            {isStealthOps ? '[TRACK SUBSCRIPTION SPENDING AND GET INTEL]' : 'Track your subscription spending and get insights'}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Tabs value={selectedTimeframe} onValueChange={(value) => setSelectedTimeframe(value as any)}>
            <TabsList className={`grid w-full grid-cols-3 ${isStealthOps ? 'tactical-surface' : ''}`}>
              <TabsTrigger value="month" className={`text-xs sm:text-sm ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                {isStealthOps ? '[MONTH]' : 'Month'}
              </TabsTrigger>
              <TabsTrigger value="quarter" className={`text-xs sm:text-sm ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                {isStealthOps ? '[QUARTER]' : 'Quarter'}
              </TabsTrigger>
              <TabsTrigger value="year" className={`text-xs sm:text-sm ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
            ...
            {upcomingPayments.length > 0 && (
              ...
              >
                <div className="flex items-center gap-3">
                  <AlertCircle className={`w-5 h-5 ${isStealthOps ? 'text-yellow-400' : 'text-orange-500'}`} />
                  <div>
                    <div className={`font-medium ${textColors.primary} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                ...
                <div className="flex items-center gap-3">
                  <div className={`w-2 h-2 ${
                    ...
                    notification.type === 'warning' 
                      ? (isStealthOps ? 'bg-yellow-400' : 'bg-yellow-500') : 
                    notification.type === 'trial' 
                      ? (isStealthOps ? 'bg-blue-400' : 'bg-blue-500') : 
                      (isStealthOps ? 'bg-green-400' : 'bg-green-500')
                  }`} 
                   style={isStealthOps ? { borderRadius: '0.125rem' } : undefined}
                  />
                  <div>
                    <div className={`font-medium ${textColors.primary} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                      {isStealthOps ? `[${notification.title.toUpperCase()}]` : notification.title}
                    </div>
                    <div className={`text-sm ${textColors.muted} line-clamp-1 ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                      {isStealthOps ? `[${notification.message.toUpperCase()}]` : notification.message}
                    </div>
                  </div>
                </div>
                <Button 
            ...
            <CardContent>
              ...
              {categoryBreakdown.length > (isMobile ? 3 : 5) && (
                <Button 
                  ...
                  style={isStealthOps ? { borderRadius: '0.125rem' } : undefined}
                >
                  {isStealthOps ? '[VIEW ALL CATEGORIES]' : 'View All Categories'} <ArrowRight className="w-3 h-3 ml-1" />
                </Button>
              )}
            </CardContent>
          </Card>
              ...
              {upcomingPayments.length > 0 ? (
                <div className="space-y-3">
                  {upcomingPayments.slice(0, 3).map((subscription) => {
                    const daysUntil = Math.ceil((new Date(subscription.nextPayment!).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
                  ...
                  {upcomingPayments.length > 3 && (
                    <Button 
                      ...
                      style={isStealthOps ? { borderRadius: '0.125rem' } : undefined}
                    >
                      {isStealthOps ? `[VIEW ALL (${upcomingPayments.length})]` : `View All (${upcomingPayments.length})`}
                    </Button>
                  )}
                </div>
              ) : (
                ...
                {highSpendingItems.map((subscription, index) => (
                  <div key={subscription.id} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className={`text-sm ${textColors.muted} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                        {isStealthOps ? `[${subscription.category.toUpperCase()}]` : subscription.category}
                      </div>
                    </div>
