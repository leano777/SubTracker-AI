...
export function SubscriptionOverview({ subscriptions }: SubscriptionOverviewProps) {
  const activeSubscriptions = subscriptions.filter(sub => sub.isActive && sub.status === 'active');
  ...
  const personalSubscriptions = activeSubscriptions.filter(sub => sub.subscriptionType === 'personal');
  const businessSubscriptions = activeSubscriptions.filter(sub => sub.subscriptionType === 'business');

  const personalTotal = personalSubscriptions.reduce((total, sub) => total + getMonthlyEquivalent(sub), 0);
  const businessTotal = businessSubscriptions.reduce((total, sub) => total + getMonthlyEquivalent(sub), 0);
