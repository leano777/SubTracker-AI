...
import { ExternalLink, Edit, Trash2, Eye, Calendar, TrendingUp, TrendingDown, XCircle, RotateCcw } from 'lucide-react';
...
export function SubscriptionTable({ 
            ...
            <div className="space-y-1">
              ...
              {subscription.variablePricing?.upcomingChanges?.map((change, index) => (
                <p key={index} className="text-xs">
                  ...
                  {change.description && <span className="block text-muted-foreground">{change.description}</span>}
                </p>
              ))}
            </div>
          </TooltipContent>
  ...
  const getCostDisplay = (subscription: Subscription) => {
    if (subscription.billingCycle === 'variable' && subscription.variablePricing?.upcomingChanges?.length) {
        ...
        return (
          <div className="space-y-1">
            ...
            <Tooltip>
              ...
              <TooltipContent>
                <p>Changes to {formatCurrency(nextChange.cost)} on {formatDate(nextChange.date)}</p>
                {nextChange.description && <p className="text-xs text-muted-foreground">{nextChange.description}</p>}
              </TooltipContent>
            </Tooltip>
          </div>
        );
        ...
        <div className="flex flex-wrap gap-4">
          ...
          <Select value={sortBy} onValueChange={setSortBy}>
            ...
            <SelectContent>
              ...
              {showCancelledActions && <SelectItem value="dateCancelled">Date Cancelled</SelectItem>}
              ...
              {sortedSubscriptions.map((subscription) => {
                ...
                return (
                  <TableRow key={subscription.id} className="group">
                    ...
                    <TableCell>
                      <Badge variant={subscription.subscriptionType === 'business' ? 'default' : 'outline'}>
