...
export function PayPeriodCalculator({ 
  ...
  const highestPeriod = payPeriods.reduce((highest, period) => 
    period.requiredAmount > (highest?.requiredAmount || 0) ? period : highest, null as PayPeriodRequirement | null);
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <div className="text-2xl font-bold">{formatCurrency(highestPeriod?.requiredAmount || 0)}</div>
            <div className="text-xs text-muted-foreground">
              {highestPeriod?.weekLabel || 'N/A'}
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {payPeriods.map((period) => {
                const isCurrentWeek = getDaysUntilDate(period.startDate) <= 0 && getDaysUntilDate(period.endDate) >= 0;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {period.subscriptions.length} subscription{period.subscriptions.length !== 1 ? 's' : ''}
                      </div>
                    </div>
                  </Button>
                );
          ...
          <CardHeader>
            ...
            {selectedPeriod && (
              <div className="text-sm text-muted-foreground">
                ...
                {getDaysUntilDate(selectedPeriod.startDate) <= 0 && getDaysUntilDate(selectedPeriod.endDate) >= 0 && (
                      ...
                      <div className="space-y-2">
                        {selectedPeriod.subscriptions
                          ...
                          .map((subscription) => {
                            ...
                            return (
                              <div key={`${subscription.id}-${subscription.dueDate}`} className="flex items-center justify-between p-3 border rounded-lg">
                                <div className="flex items-center space-x-3">
                                  <div>
                                    ...
                                    <div className="text-sm text-muted-foreground">
                                      ...
                                      {daysUntil <= 7 && (
                                        <span className={`ml-2 px-2 py-0.5 rounded-full text-xs text-white ${getPriorityColor(daysUntil)}`}>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {upcomingPayments.slice(0, 6).map((payment, index) => (
                <div key={`${payment.id}-${payment.dueDate}-${index}`} className="flex items-center justify-between p-3 border rounded-lg">
                  <div className="flex items-center space-x-2">
                    <div>
                  ...
                  <div className="text-right">
                    ...
                    <span className={`px-1.5 py-0.5 rounded-full text-xs text-white ${getPriorityColor(payment.daysUntil)}`}>
                      {payment.daysUntil}d
                    </span>
                  </div>
                </div>
