...
import { Settings, Bell, DollarSign, Palette, Shield, Download, Upload, RotateCcw, Save, Moon, Sun, Zap, Lightbulb } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className={`flex items-center space-x-2 ${textColors.primary} ${isStealthOps ? 'font-mono tracking-wide tactical-text-glow' : ''}`}>
          ...
          <p className={`${textColors.muted} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
            {isStealthOps ? '[CUSTOMIZE YOUR SUBTRACKER EXPERIENCE]' : 'Customize your subscription tracker experience'}
          </p>
        </div>
      ...
      {hasChanges && (
        ...
        >
          <p className={`text-sm ${
            ...
            {isStealthOps ? '[UNSAVED CHANGES DETECTED - REMEMBER TO SAVE]' : 'You have unsaved changes. Don\'t forget to save your settings.'}
          </p>
        </div>
      )}
      ...
      <Card className={isStealthOps ? 'tactical-surface' : ''}>
        <CardHeader>
          ...
          <CardDescription className={`${textColors.muted} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
            {isStealthOps ? '[CONFIGURE ALERT PARAMETERS]' : 'Configure when and how you want to be notified about your subscriptions'}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                <Label htmlFor="upcomingPayments" className={`${textColors.primary} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                  {isStealthOps ? '[UPCOMING PAYMENT ALERTS]' : 'Upcoming Payment Alerts'}
                </Label>
                <p className={`text-sm ${textColors.muted} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                  {isStealthOps ? '[ALERT BEFORE PAYMENTS DUE]' : 'Get notified before subscription payments are due'}
                </p>
              </div>
              <Switch
                id="upcomingPayments"
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                <Label htmlFor="highSpending" className={`${textColors.primary} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                  {isStealthOps ? '[HIGH SPENDING ALERTS]' : 'High Spending Alerts'}
                </Label>
                <p className={`text-sm ${textColors.muted} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                  {isStealthOps ? '[ALERT WHEN THRESHOLD EXCEEDED]' : 'Get alerted when spending exceeds your threshold'}
                </p>
              </div>
              <Switch
                id="highSpending"
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                <Label htmlFor="trialExpirations" className={`${textColors.primary} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                  {isStealthOps ? '[TRIAL EXPIRATION ALERTS]' : 'Free Trial Expiration Alerts'}
                </Label>
                <p className={`text-sm ${textColors.muted} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                  {isStealthOps ? '[ALERT BEFORE TRIALS EXPIRE]' : 'Get notified when free trials are about to expire'}
                </p>
              </div>
              <Switch
                id="trialExpirations"
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                <Label htmlFor="weeklyReports" className={`${textColors.primary} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                  {isStealthOps ? '[WEEKLY SUMMARY REPORTS]' : 'Weekly Summary Reports'}
                </Label>
                <p className={`text-sm ${textColors.muted} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                  {isStealthOps ? '[WEEKLY INTEL REPORTS]' : 'Receive weekly spending summaries and insights'}
                </p>
              </div>
              <Switch
                id="weeklyReports"
          ...
          <div className="space-y-4">
            <h4 className={`font-medium ${textColors.primary} ${isStealthOps ? 'font-mono tracking-wide tactical-text-glow' : ''}`}>
              {isStealthOps ? '[ALERT TIMING]' : 'Alert Timing'}
            </h4>
            
            <div className="space-y-2">
              <Label htmlFor="upcomingPaymentDays" className={`${textColors.primary} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
              ...
              <div className="flex items-center space-x-4">
                ...
                <Badge 
                  variant="outline" 
                  className={`min-w-fit ${
                    isStealthOps ? 'tactical-surface border border-gray-600 font-mono tracking-wide' : ''
                  }`}
                  style={isStealthOps ? { borderRadius: '0.125rem' } : undefined}
                >
              ...
              <p className={`text-xs ${textColors.muted} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                {isStealthOps ? '[ALERT DAYS BEFORE PAYMENTS DUE]' : 'Get notified this many days before payments are due'}
              </p>
            </div>

            <div className="space-y-2">
              <Label htmlFor="trialReminderDays" className={`${textColors.primary} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
              ...
              <p className={`text-xs ${textColors.muted} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                {isStealthOps ? '[ALERT DAYS BEFORE TRIALS EXPIRE]' : 'Get notified this many days before trials expire'}
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <Card className={isStealthOps ? 'tactical-surface' : ''}>
        <CardHeader>
          ...
          <CardDescription className={`${textColors.muted} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
            {isStealthOps ? '[SET SPENDING LIMITS AND BUDGET ALERTS]' : 'Set spending limits and budget alerts'}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="highSpendingAmount" className={`${textColors.primary} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
              {isStealthOps ? '[HIGH SPENDING ALERT THRESHOLD]' : 'High Spending Alert Threshold'}
            </Label>
            <div className="flex items-center space-x-2">
              <div className="relative flex-1">
                <span className={`absolute left-3 top-1/2 transform -translate-y-1/2 ${textColors.muted}`}>$</span>
                <Input
                  ...
                  onChange={(e) => 
                    updateLocalSettings({
                      thresholds: { ...localSettings.thresholds, highSpendingAmount: parseFloat(e.target.value) || 0 }
                    })
                  }
                  className={`pl-8 ${
                    isStealthOps 
                      ? 'tactical-input font-mono tracking-wide' 
                      : ''
                  }`}
                  style={isStealthOps ? { borderRadius: '0.125rem' } : undefined}
                />
            ...
            <p className={`text-xs ${textColors.muted} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
              {isStealthOps ? '[ALERT WHEN MONTHLY COSTS EXCEED THIS AMOUNT]' : 'Get alerted when your monthly subscription costs exceed this amount'}
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className={isStealthOps ? 'tactical-surface' : ''}>
        <CardHeader>
          ...
          <CardDescription className={`${textColors.muted} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
            {isStealthOps ? '[CUSTOMIZE APP APPEARANCE AND BEHAVIOR]' : 'Customize how the app looks and behaves'}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label htmlFor="defaultView" className={`${textColors.primary} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
              ...
              >
                ...
                <SelectContent 
                  className={`${
                    isStealthOps ? 'tactical-surface border border-gray-600' : ''
                  }`}
                  style={isStealthOps ? { borderRadius: '0.125rem' } : undefined}
                >
                  <SelectItem value="dashboard">{isStealthOps ? '[DASHBOARD]' : 'Dashboard'}</SelectItem>
                  <SelectItem value="active">{isStealthOps ? '[ACTIVE SUBSCRIPTIONS]' : 'Active Subscriptions'}</SelectItem>
                  <SelectItem value="calendar">{isStealthOps ? '[CALENDAR VIEW]' : 'Calendar View'}</SelectItem>
                  <SelectItem value="pay-periods">{isStealthOps ? '[PAY PERIODS]' : 'Pay Periods'}</SelectItem>
                  <SelectItem value="watchlist">{isStealthOps ? '[WATCHLIST]' : 'Watchlist'}</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  <Label htmlFor="showFavicons" className={`${textColors.primary} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                    {isStealthOps ? '[SHOW SERVICE LOGOS]' : 'Show Service Logos'}
                  </Label>
                  <p className={`text-xs ${textColors.muted} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                    {isStealthOps ? '[DISPLAY SERVICE ICONS]' : 'Display service icons throughout the app'}
                  </p>
                </div>
                <Switch
                  id="showFavicons"
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                <Label htmlFor="showCancelled" className={`${textColors.primary} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                  {isStealthOps ? '[SHOW CANCELLED SUBSCRIPTIONS]' : 'Show Cancelled Subscriptions'}
                </Label>
                <p className={`text-xs ${textColors.muted} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                  {isStealthOps ? '[INCLUDE CANCELLED IN OVERVIEW]' : 'Include cancelled subscriptions in overview'}
                </p>
              </div>
              <Switch
                id="showCancelled"
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                <Label htmlFor="groupByCategory" className={`${textColors.primary} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                  {isStealthOps ? '[GROUP BY CATEGORY]' : 'Group by Category'}
                </Label>
                <p className={`text-xs ${textColors.muted} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                  {isStealthOps ? '[GROUP SUBSCRIPTIONS BY CATEGORY]' : 'Group subscriptions by category in lists'}
                </p>
              </div>
              <Switch
                id="groupByCategory"
            ...
            <div className="space-y-2">
              <Label htmlFor="theme" className={`${textColors.primary} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                {isStealthOps ? '[THEME]' : 'Theme'}
              </Label>
              <div className="grid grid-cols-3 gap-2">
                <button
                  onClick={() => updateLocalSettings({
                    preferences: { ...localSettings.preferences, theme: 'light', darkMode: false }
                  })}
                ...
                >
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <span className={`text-sm font-medium ${textColors.primary} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                ...
                <button
                  onClick={() => updateLocalSettings({
                    preferences: { ...localSettings.preferences, theme: 'stealth-ops', darkMode: false }
                  })}
                  className={`p-3 border-2 transition-all duration-200 ${
                    isStealthOps ? '' : 'rounded-lg'
                  } ${
                ...
                >
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <span className={`text-sm font-medium ${isStealthOps ? 'text-green-400 font-mono tracking-wide' : ''}`}>
              ...
              <p className={`text-xs ${textColors.muted} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                {isStealthOps ? '[CHOOSE INTERFACE THEME - STEALTH OPS FEATURES HIGH-CONTRAST TACTICAL STYLING]' : 'Choose your interface theme. Stealth Ops features high-contrast tactical styling for maximum readability.'}
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className={`${textColors.muted} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
              {isStealthOps ? '[BACKUP, RESTORE, AND MANAGE YOUR SUBSCRIPTION DATA]' : 'Backup, restore, and manage your subscription data'}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          >
            <CardContent className="p-6 text-center space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className={`text-sm ${textColors.muted} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                  {isStealthOps ? '[DOWNLOAD ALL YOUR SUBSCRIPTION DATA]' : 'Download all your subscriptions, cards, and settings'}
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          >
            <CardContent className="p-6 text-center space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className={`text-sm ${textColors.muted} ${isStealthOps ? 'font-mono tracking-wide' : ''}`}>
                  {isStealthOps ? '[RESTORE FROM BACKUP OR CSV]' : 'Restore from a backup file or import CSV data'}
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card 
            ...
            onClick={() => {
              if (confirm(isStealthOps ? '[CONFIRM: DELETE ALL DATA? THIS CANNOT BE UNDONE]' : 'Are you sure you want to reset all data? This action cannot be undone.')) {
                onResetApp();
              }
            }}
            style={isStealthOps ? { borderRadius: '0.125rem' } : undefined}
            ...
            <CardContent className="p-6 text-center space-y-4">
              ...
              <div className="space-y-2">
                <h3 className={`font-semibold ${
                  isStealthOps ? 'text-red-400 font-mono tracking-wide tactical-text-glow' : 'text-red-700 dark:text-red-300'
                }`}>
                  {isStealthOps ? '[RESET APP]' : 'Reset App'}
                </h3>
                <p className={`text-sm ${
                  isStealthOps ? 'text-red-400 font-mono tracking-wide' : 'text-red-600 dark:text-red-400'
                }`}>
                  {isStealthOps ? '[CLEAR ALL DATA AND RESET TO DEFAULTS]' : 'Clear all data and reset to default settings'}
                </p>
              </div>
            </CardContent>
          </Card>
