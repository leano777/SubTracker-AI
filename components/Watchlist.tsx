...
export function Watchlist({ 
  ...
  const totalPotentialSpending = watchlistItems.reduce((total, item) => {
    ...
    switch (item.billingCycle) {
      case 'monthly': return total + item.cost;
      case 'quarterly': return total + (item.cost / 3);
      case 'yearly': return total + (item.cost / 12);
      default: return total;
    }
  }, 0);

  const priorityCounts = watchlistItems.reduce((acc, item) => {
    const priority = item.priority || 'low';
    acc[priority] = (acc[priority] || 0) + 1;
    return acc;
  }, {} as Record<string, number>);

  const categoryBreakdown = watchlistItems.reduce((acc, item) => {
      ...
      switch (sortBy) {
        ...
        case 'cost':
          const getMonthlyCost = (item: Subscription) => {
            ...
            switch (item.billingCycle) {
              case 'monthly': return item.cost;
              case 'quarterly': return item.cost / 3;
              case 'yearly': return item.cost / 12;
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {Object.keys(categoryBreakdown).length > 0 ? 
                Object.entries(categoryBreakdown).sort(([,a], [,b]) => b - a)[0]?.[1] || 0 : 0}
            </div>
            <p className="text-xs text-muted-foreground">
              {Object.keys(categoryBreakdown).length > 0 ? 
                Object.entries(categoryBreakdown).sort(([,a], [,b]) => b - a)[0]?.[0] || 'None' : 'None'}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold">
            Subscription Watchlist ({filteredItems.length})
          </h2>
          <p className="text-sm text-muted-foreground">
            Services you're interested in trying
          </p>
        </div>
