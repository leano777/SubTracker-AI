...
import { Calendar, DollarSign, TrendingUp, AlertTriangle, Calculator, Clock, ChevronRight, Info } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Badge variant="secondary" className="bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300">
            ...
            Next Thursday: {currentThursday.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
          </Badge>
        </div>
      </div>
      ...
      {error && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            Error calculating pay periods: {error}
          </AlertDescription>
        </Alert>
      )}
          ...
          {requirements.length > 0 ? (
            <div className="space-y-3">
              {requirements.map((period, index) => {
                ...
                const urgency = period.requiredAmount > averageWeekly * 1.5 ? 'high' : 
                               period.requiredAmount > averageWeekly * 1.2 ? 'medium' : 'normal';
                          ...
                          {period.subscriptions.map((subscription) => (
                            ...
                            >
                              ...
                              <div className="text-right">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {subscription.dueDate && new Date(subscription.dueDate).toLocaleDateString('en-US', {
                                    month: 'short',
                                    day: 'numeric'
                                  })}
                                </div>
        ...
        <Alert>
          ...
          <AlertDescription className="space-y-2">
            ...
            <ul className="text-sm space-y-1 ml-4">
              ...
              <li>• Plan extra for week of {requirements.find(r => r.requiredAmount === highestWeek)?.weekLabel || 'highest spending'} ({formatCurrency(highestWeek)})</li>
              <li>• Consider moving some subscriptions to spread costs more evenly</li>
              {stats.active > 0 && <li>• You have {stats.active} active subscription{stats.active !== 1 ? 's' : ''} to track</li>}
            </ul>
          </AlertDescription>
        </Alert>
      )}
